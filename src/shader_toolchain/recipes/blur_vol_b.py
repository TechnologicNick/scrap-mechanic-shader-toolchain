"""Recognize and emit the depth-aware volumetric upsample blur pass."""
...
def apply_blur_vol_b_recipe(
    staging: Path,
    records: list[dict[str, Any]],
    blobs: list[bytes],
    compiler: Any,
) -> dict[str, Any] | None:
    shaders = [record for record in records if record["source_name"] == "blur_vol_b"]
    ...
    return emit_validated_module(
        ...
        blobs,
        compiler,
        recipe_name="blur_vol_b",
        bodies={shader["selector"]: asset("blur_vol_b_pixel.hlsl")},
