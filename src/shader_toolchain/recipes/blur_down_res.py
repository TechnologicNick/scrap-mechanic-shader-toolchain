"""Recognize and emit the readable four-tap downsample blur."""
...
def apply_blur_down_res_recipe(
    staging: Path,
    records: list[dict[str, Any]],
    blobs: list[bytes],
    compiler: Any,
) -> dict[str, Any] | None:
    shaders = [record for record in records if record["source_name"] == "blur_down_res"]
    ...
    return emit_validated_module(
        ...
        blobs,
        compiler,
        recipe_name="blur_down_res",
        bodies={shader["selector"]: asset("blur_down_res_pixel.hlsl")},
