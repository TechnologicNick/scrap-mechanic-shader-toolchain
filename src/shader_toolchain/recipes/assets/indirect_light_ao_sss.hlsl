...
IndirectLightAoSurface LoadIndirectLightAoSurface(float2 unscaledUv)
{
  ...
  float4 gatheredProfiles = tMaterial.Gather(
      PointClampClamp_s, surface.scaledUv);
  uint4 neighborhoodProfiles =
      (uint4)(gatheredProfiles * 255.0 + 0.5) & 7u;
  surface.profile = any(neighborhoodProfiles == 2u)
      ? 2u : ((uint)(surface.material.z * 255.0 + 0.5) & 7u);
#else
...
float3 LoadIndirectLightAoOnlyViewPosition(float2 scaledUv, uint2 aoSize)
{
  ...
  float depth = DecodeIndirectLightAoDepth(tAoDepth.SampleLevel(
