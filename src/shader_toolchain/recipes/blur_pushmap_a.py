"""Recognize and emit the first push-map blur and dilation pass."""
...
def apply_blur_pushmap_a_recipe(
    staging: Path,
    records: list[dict[str, Any]],
    blobs: list[bytes],
    compiler: Any,
) -> dict[str, Any] | None:
    shaders = [record for record in records if record["source_name"] == "blur_pushmap_a"]
    ...
    return emit_validated_module(
        ...
        blobs,
        compiler,
        recipe_name="blur_pushmap_a",
        bodies={shader["selector"]: asset("blur_pushmap_a_pixel.hlsl")},
