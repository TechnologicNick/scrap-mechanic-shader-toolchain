"""Recognize and emit the readable cloud upsample copy shader."""
...
def apply_upres_clouds_recipe(
    staging: Path,
    records: list[dict[str, Any]],
    blobs: list[bytes],
    compiler: Any,
) -> dict[str, Any] | None:
    shaders = [record for record in records if record["source_name"] == "upres_clouds"]
    ...
    return emit_validated_module(
        ...
        blobs,
        compiler,
        recipe_name="upres_clouds",
        bodies={shader["selector"]: asset("upres_clouds_pixel.hlsl")},
