"""Recognize and emit the readable weighted bloom downsample shader."""
...
def apply_bloom_downres_recipe(
    staging: Path,
    records: list[dict[str, Any]],
    blobs: list[bytes],
    compiler: Any,
) -> dict[str, Any] | None:
    shaders = [record for record in records if record["source_name"] == "bloom_downres"]
    ...
    return emit_validated_module(
        ...
        blobs,
        compiler,
        recipe_name="bloom_downres",
        bodies={shader["selector"]: asset("bloom_downres_pixel.hlsl")},
