from shader_toolchain.recipes.main_part import (
    ...
    is_main_part_legacy_glass_surface_multi_medium,
    lift_main_part_legacy_glass_surface_multi_medium,
)


def test_legacy_glass_surface_multi_medium_policy_is_exact() -> None:
    defines = sorted(LEGACY_GLASS_SURFACE_MULTI_MEDIUM_DEFINES)
    assert is_main_part_legacy_glass_surface_multi_medium(defines)
    assert not is_main_part_legacy_glass_surface_multi_medium(
...
def test_legacy_glass_surface_lift_factors_large_runtime_abis() -> None:
...
'''
    lifted = lift_main_part_legacy_glass_surface_multi_medium(source)
    for filename in (
        "main_part_projection_abi.hlsl", "main_part_perframe_abi.hlsl",
        "main_part_reflections_abi.hlsl", "main_part_cluster_abi.hlsl",
        "main_part_lightprops_abi.hlsl",
    ):
        assert f'#include "include/{filename}"' in lifted
    assert '#include "include/main_part_legacy_glass_multi.hlsl"' in lifted
    function_body = lifted[lifted.index("void commonPS(") :]
    assert "#include" not in function_body
    assert "EvaluateMainPartLegacyGlassMultiLighting" in function_body
