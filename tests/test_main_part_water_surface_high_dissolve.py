from shader_toolchain.recipes.main_part import (
    ...
    is_main_part_water_surface_single_high_dissolve,
    lift_main_part_water_surface_single_high_dissolve,
)


def test_water_surface_single_high_dissolve_policy_is_exact() -> None:
    defines = sorted(WATER_SURFACE_SINGLE_HIGH_DISSOLVE_DEFINES)
    assert is_main_part_water_surface_single_high_dissolve(defines)
    assert not is_main_part_water_surface_single_high_dissolve(
        [define for define in defines if define != "PS_FBDRF_DIF"]
    )
    assert not is_main_part_water_surface_single_high_dissolve(
        defines + ["PS_REFLECTION_MULTI"]
    )


def test_high_dissolve_water_lift_uses_shared_abis_and_fidelity_body() -> None:
...
'''
    lifted = lift_main_part_water_surface_single_high_dissolve(source)
    for filename in (
        "main_part_projection_abi.hlsl", "main_part_perframe_abi.hlsl",
        "main_part_dissolve_abi.hlsl", "main_part_cluster_abi.hlsl",
    ...
    assert "EvaluateMainPartWaterSurfaceSingleHighDissolve" in lifted
