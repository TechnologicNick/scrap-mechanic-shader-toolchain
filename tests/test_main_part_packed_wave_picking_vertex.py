from shader_toolchain.recipes.main_part import (
    ...
    is_main_part_packed_wave_picking_scroll,
    lift_main_part_packed_wave_picking_scroll,
)


def test_packed_wave_picking_policy_is_exact() -> None:
    defines = sorted(PACKED_WAVE_PICKING_SCROLL_DEFINES)
    assert is_main_part_packed_wave_picking_scroll(defines)
    assert not is_main_part_packed_wave_picking_scroll(
        [define for define in defines if define != "VS_PICKING_BUFFER"]
    )
    assert not is_main_part_packed_wave_picking_scroll(
...
def test_packed_wave_picking_lift_reuses_packed_transform_model() -> None:
...
cbuffer CB_PICKING : register(b13) { uint4 ids[256]; }
...
'''
    lifted = lift_main_part_packed_wave_picking_scroll(source)
    for filename in (
    ...
    assert '#include "include/main_part_packed_wave_picking_vertex.hlsl"' in lifted
    assert "EvaluateMainPartPackedWavePickingVertex" in lifted
